...
export default async function handler(req, res) {
  try {
    ...
    if (["GET", "POST"].includes(req.method) && postbackInput.event) {
      return await handlePostback(req, res, postbackInput);
    }
...
async function handleAccessStatus(req, res) {
  ...
  if (!databaseConfigured()) {
    return res.status(200).json({ ok:true, access:{ configured:false, registered:false, deposit_amount:0, deposit_ok:false, allowed:false, min_deposit:minDeposit } });
  }
...
async function handlePostback(req, res, input) {
  ...
  if (["registration", "register", "reg"].includes(event)) {
    ...
    console.info("Postback accepted", { event:"registration" });
    ...
    console.info("Postback accepted", { event:"firstdep", amount });
  } else {
...
async function handleAnalysis(req, res) {
  ...
  return res.status(200).json({
    ok:true,
    result,
    meta:{ timeframe, expiration_minutes:Number(expiration), locale, language:language.name, model, min_confidence:minConfidence, disclaimer:"AI chart analysis does not guarantee the outcome of a trade." }
  });
}
...
function readPostbackInput(req) {
  ...
  if (typeof body === "string") {
    try { body = Object.fromEntries(new URLSearchParams(body).entries()); }
    catch { body = {}; }
  }
...
function getPool() {
  if (!databaseConfigured()) {
    const error = new Error("database_not_configured"); error.code = "database_not_configured"; throw error;
  }
  if (!pool) {
...
async function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = getPool().query(`CREATE TABLE IF NOT EXISTS ai_signal_access (
...
async function getAccessStatus(userId, minDeposit) {
  await ensureSchema();
  const result = await getPool().query(`SELECT registered, registered_at, deposit_amount, deposit_ok, deposit_at FROM ai_signal_access WHERE telegram_id=$1 LIMIT 1`, [String(userId)]);
...
async function upsertRegistration(userId) {
  await ensureSchema();
  await getPool().query(`INSERT INTO ai_signal_access (telegram_id,registered,registered_at,updated_at)
...
async function upsertDeposit(userId, amount, minDeposit) {
  ...
  await getPool().query(`INSERT INTO ai_signal_access (telegram_id,registered,registered_at,deposit_amount,deposit_ok,deposit_at,updated_at)
    ...
    ON CONFLICT (telegram_id) DO UPDATE SET
      ...
      updated_at=NOW()`, [String(userId),amount,amount >= minDeposit]);
}
...
function formatAmount(amount) { return Number(amount).toFixed(2).replace(/\.00$/,""); }
