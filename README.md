...
На читаемом свечном графике сервер возвращает ближайшее направление `ВВЕРХ` или `ВНИЗ`, а силу видимых подтверждений показывает процентом уверенности. `НЕТ СИГНАЛА` остаётся для нечитаемых, критично обрезанных и не содержащих свечной график изображений.
...
- уверенность показывает силу направления на текущем скриншоте и не является статистической точностью;
...
команды также разрешены администраторам этой группы. Список команд идемпотентно синхронизируется
при первом update бота после деплоя; повторный вызов installer не нужен.
...
Vercel Cron вызывает `/api/follow-up` ежедневно. Если задан `CRON_SECRET`, он передаётся в заголовке
Authorization и проверяется строго; без него маршрут принимает только точное значение официального
`x-vercel-cron-schedule` из конфигурации проекта.
